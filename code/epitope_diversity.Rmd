...
knitr::opts_knit$set(root.dir = "C:/Users/cf199/Box/O\'Meara-Taylor Postdoc/Once Bitten")
...
csp_seqs <- Biostrings::readDNAStringSet("epitopes/MozzieAminoAcidAnalysis/spat21_CSP_uniqueSeqs_final_censored.fasta")


csp_haplotypes<-read.csv("epitopes/MozzieAminoAcidAnalysis/Haplotype Data for Haplotype Prevalence/Summarized data/spat21_csp_summarized_haplotype_list_31DEC2019.csv")
...
csp_MergedData<-merge(HDS,csp_MergHaploAA,by="sample_name_dbs") %>%
  ...
  rowwise() %>%
  mutate(cd4_epitope = paste0(substr(x,25,25),substr(x,28,28), substr(x,29,29), substr(x,31,31)), cd8_epitope = paste0(substr(x,59,59),substr(x,64,64), substr(x,66,66), substr(x,68,68)))
```
...
ggsave(plot = cd4_sunburst_plot, "epitopes/MozzieAminoAcidAnalysis/cfm_figures/cd4_sunburst.png", width = 6, height = 6)
...
ggsave(plot = cd8_sunburst_plot, "epitopes/MozzieAminoAcidAnalysis/cfm_figures/cd8_sunburst.png", width = 6, height = 6)
...
ama_seqs <- Biostrings::readDNAStringSet("epitopes/MozzieAminoAcidAnalysis/spat21_AMA_uniqueSeqs_final_censored.fasta")


ama_haplotypes<-read.csv("epitopes/MozzieAminoAcidAnalysis/Haplotype Data for Haplotype Prevalence/Summarized data/spat21_ama_summarized_haplotype_list_31DEC2019.csv")
...
ggsave(plot = ama_sunburst_plot, "epitopes/MozzieAminoAcidAnalysis/cfm_figures/ama_sunburst.png", width = 6, height = 6)
...
csp_moi <- csp_MergedData %>%
  ...
  summarise(HType_moi = mean(haplotype_number), cd4_moi = n_distinct(cd4_epitope), cd8_moi = n_distinct(cd8_epitope))
