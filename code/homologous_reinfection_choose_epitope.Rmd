...
#CSP B-cell DV10
# CurrentEpitopeName<-("DV10")
# EpitopeCurrent<-c("x5","x6","x8","x10")
# AMA <- FALSE
# prev <- FALSE
...
## Should MOI be included in the models?
```{r}

model_moicont <- coxme(Surv(Days,EventCensored) ~ symptomatic_bin + gender + ageCat + freq+mosquito_prev_2weeks_cat+epitopecount+(1|unq_memID), data = TimeToDataAdvEpitope_60, ties = "efron")

model_moicont
exp(confint(model_moicont))

model_moicat <- coxme(Surv(Days,EventCensored) ~ symptomatic_bin + gender + ageCat + freq+mosquito_prev_2weeks_cat+epitopeCat+(1|unq_memID), data = TimeToDataAdvEpitope_60, ties = "efron")

model_moicat
exp(confint(model_moicat))
...
extractAIC(model) #model without moi
extractAIC(model_moicont) #model with continuous moi
extractAIC(model_moicat) #model with categorical moi


#likelihood ratio tests

# no moi vs continuous moi
anova(model_moicont, model)

# no moi vs categorical moi
anova(model_moicat, model)
...
Let's look at our rare epitopes a little closer
```{r eval = prev}
#How many rare epitopes were singletons or doubletons
TimeToDataAdvEpitope_60 %>%
  filter(quant == 1) %>%
  group_by(AAs) %>%
  count() %>%
...
# How many rare types were observed in polyclonal vs monoclonal infections?
# Let's look at the epitope moi for each quantile
TimeToDataAdvEpitope_60 %>%
  ggplot() +
  aes(x = factor(quant, levels = c(3,2,1), labels = c("Common", "Middling", "Rare")), y = epitopecount) +
  labs(x = "", y = "Epitope MOI") +
  geom_boxplot() +
...
TimeToDataAdvEpitope_60 %>%
  group_by(quant) %>%
  mutate(total_epitope_occurences = n()) %>%
  ungroup() %>%
  filter(epitopecount == 1) %>%
  group_by(quant) %>%
  summarise(prop_mono = n()/total_epitope_occurences, se = sqrt(prop_mono*(1-prop_mono)/total_epitope_occurences), lower = prop_mono - se, upper = prop_mono + se) %>%
  unique()

library(gtsummary)
TimeToDataAdvEpitope_60 %>%
  select(quant, epitopecount, epitopeCat) %>%
  mutate(quant = factor(quant,levels = c(3,2,1), labels = c("Common", "Middling", "Rare") )) %>%
...
We want to know read support for common, middling, and rare epitope types
csp
```{r eval= (!AMA & prev)}
Htype_reads <- read_csv("data/original/spat21_csp_haplotype_table_censored_final_version_with_moi_and_ids_CLEANVERSION_30SEPT2019.csv") %>%
  select(-MiSeq.ID, -Run, -sample_type, -haplotype_number, -haplotype_reads) %>%
  filter(sample_name_dbs %in% TimeToDataAdvEpitope_60$sample_name_dbs) %>%
  pivot_longer(2:299, names_to = "HType", values_to = "reads") %>%
  filter(reads > 0)


AAs_Htype_key <- MergHaploAA %>% 
  filter(sample_name_dbs %in% TimeToDataAdvEpitope_60$sample_name_dbs) %>%
  select(HType, sample_name_dbs, c(EpitopeCurrent)) %>%
  unite("AAs",3:(2+length(EpitopeCurrent)), sep = "", remove = FALSE)

read_support <- AAs_Htype_key %>%
  left_join(Htype_reads) %>%
  left_join(TimeToDataAdvEpitope_60 %>%
              select(AAs, quant) %>%
              distinct())
...
ama1
```{r eval=AMA}
Htype_reads <- read_csv("data/original/spat21_AMA_haplotype_table_censored_final_version_with_moi_and_ids_CLEANVERSION_15OCT2019.csv") %>%
  select(-MiSeq.ID, -Run, -sample_type, -haplotype_number, -haplotype_reads) %>%
  filter(sample_name_dbs %in% TimeToDataAdvEpitope_60$sample_name_dbs) %>%
  pivot_longer(2:299, names_to = "HType", values_to = "reads") %>%
  filter(reads > 0)
...
AAs_Htype_key <- MergHaploAA %>% 
  filter(sample_name_dbs %in% TimeToDataAdvEpitope_60$sample_name_dbs) %>%
  select(HType, sample_name_dbs, c(EpitopeCurrent)) %>%
  unite("AAs",3:6, sep = "", remove = FALSE)
...
comparing read support between common, middling, and rare types
```{r eval = prev}
read_support %>%
  ggplot() +
  aes(x = factor(quant, levels = c(3,2,1), labels = c("Common", "Middling", "Rare")), y = reads) +
  scale_y_log10() +
  labs(x = "", y = "Reads") +
  geom_boxplot() +
  theme_bw()
...
#Bring in haplotype data
  ...
  mutate(ageCat = case_when(age_y_baseline <= 5 ~ "<5",
    ...
    mutate(ageCat=factor(ageCat,levels=c("<5","5-15",">15"))) %>%
  mutate(epitopeCat = case_when(epitopecount == 1 ~"1",
                                epitopecount >1 & epitopecount <= 5 ~ "2-5",
                                epitopecount > 5 ~ "> 5"),
         epitopeCat = factor(epitopeCat, levels = c("1", "2-5", "> 5")))

epitope_frequencies <- for_perm %>%
  separate_rows(AAs) %>%
  group_by(AAs) %>%
  summarise(n = n()) %>%
  mutate(freq = n/sum(n))
...
for_perm %>%
  ...
  aes(x = epitopeCat) +
  geom_bar() +
  labs(x = "Epitope type count", y = "Number of samples") +
  theme_bw()


# sample_AAs <- for_perm %>%
#   separate_rows(AAs) %>%
#   select(sample_name_dbs, AAs)
...
# perm <- function(data = for_perm) {
#   
#   AAsrandom <- split(sample(sample_AAs$AAs), sample_AAs$sample_name_dbs) %>%
#     stack() %>%
#     rename("sample_name_dbs" = "ind", "AAsrandom" = "values") %>%
#     group_by(sample_name_dbs) %>%
#     summarise(AAsrandom = str_c(AAsrandom, collapse = ","))
#   
#   perm_data <- for_perm %>%
#     left_join(AAsrandom) %>%
#     separate_rows(AAsrandom) %>%
...
```
## How do epitope frequencies compare in permuted datasets vs actual data?
```{r}
perm_freqs <- perm_data %>%
  bind_rows(.id = "perm") %>%
  group_by(perm, AAsrandom) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  group_by(perm) %>%
...
ggplot() +
  geom_density(data = perm_freqs, mapping = aes(x = freq, group = perm), position = "identity", color = "lightgrey", alpha = 0.3, adjust = 1) +
  geom_density(data = epitope_frequencies %>% mutate(freq = freq*100), mapping = aes(x = freq), position = "identity", color = "black", adjust = 1) +
  theme_bw() +
  theme(legend.position = "none")
```
...
homol_random_logrank_med <- homol_random_logrank_stats %>%
  summarise(med_chisq = median(chisq), med_p = median(p.value))

homol_random_logrank_med
...
homol_random_fig <- surv_data_forfig %>%
  ...
  annotate("rect", xmin = 240, xmax = 365, ymin = 0.01, ymax = 0.18, color = "black", fill = "white") +
  annotate("text", x = 300, y = 0.13, label = paste0("Median \u03a7\u00b2 = ", round(homol_random_logrank_med$med_chisq,3)))+ 
  annotate("text", x = 305, y = 0.06, label = paste0("Median p = ", round(homol_random_logrank_med$med_p ,4))) +
...
## Homologous v random Asx/Sx
We want to see if there is a difference between homolgous and random reinfection after symptomatic exposure separately for Asx and Sx index episodes
```{r}
# First get the homologous survival curves separately for Asx and Sx

## Asx
surv_data_homol_Asx <- survfit(Surv(Days,EventCensored)~1,data=TimeToDataAdvEpitope_60 %>% filter(symptomatic_status == "asymptomatic infection"))

surv_data_homol_Asx
summary(surv_data_homol_Asx, times = 365.25)

surv_homol_Asx <- fortify(surv_data_homol_Asx)

## Sx
surv_data_homol_Sx <- survfit(Surv(Days,EventCensored)~1,data=TimeToDataAdvEpitope_60 %>% filter(symptomatic_status == "symptomatic infection"))

surv_data_homol_Sx
summary(surv_data_homol_Sx, times = 365.25)

surv_homol_Sx <- fortify(surv_data_homol_Sx)


# Now let's get the same curves for the permuted datasets and make some plots

## Asx
perm_surv_Asx <- lapply(1:1000, function(x){
  survfit(Surv(Days,EventCensored)~1,data=perm_data[[x]] %>% filter(symptomatic_status == "asymptomatic infection")) %>%
  fortify()
}) %>%
  bind_rows(.id = "perm")


random_Asx_avg <- perm_surv_Asx %>%
  group_by(time) %>%
  summarise(med_surv = median(surv), mean_surv = mean(surv), min_surv = min(surv), max_surv = max(surv), lowerCI = median(lower), upperCI = median(upper))

random_Asx_forfig <- random_Asx_avg %>%
  arrange(time) %>%
  mutate(time = lead(time, 1)) %>%
  bind_rows(random_Asx_avg)


surv_Asx_data_forfig <- surv_homol_Asx %>%
  arrange(time) %>%
  mutate(time = lead(time, 1)) %>%
  bind_rows(surv_homol_Asx) %>%
  rename(med_surv = surv, lowerCI = lower, upperCI = upper) %>%
  select(time, med_surv, lowerCI, upperCI) %>%
  mutate(dataset = "Homologous") %>%
  bind_rows(random_Asx_forfig %>%
          select(time, med_surv, lowerCI, upperCI) %>%
          mutate(dataset = "Random"))
...
perm_surv_Sx <- lapply(1:1000, function(x){
  survfit(Surv(Days,EventCensored)~1,data=perm_data[[x]] %>% filter(symptomatic_status == "symptomatic infection")) %>%
  fortify()
}) %>%
  bind_rows(.id = "perm")


random_Sx_avg <- perm_surv_Sx %>%
  group_by(time) %>%
  summarise(med_surv = median(surv), mean_surv = mean(surv), min_surv = min(surv), max_surv = max(surv), lowerCI = median(lower), upperCI = median(upper))

random_Sx_forfig <- random_Sx_avg %>%
  arrange(time) %>%
  mutate(time = lead(time, 1)) %>%
  bind_rows(random_Sx_avg)


surv_Sx_data_forfig <- surv_homol_Sx %>%
  arrange(time) %>%
  mutate(time = lead(time, 1)) %>%
  bind_rows(surv_homol_Sx) %>%
  rename(med_surv = surv, lowerCI = lower, upperCI = upper) %>%
  select(time, med_surv, lowerCI, upperCI) %>%
  mutate(dataset = "Homologous") %>%
  bind_rows(random_Sx_forfig %>%
          select(time, med_surv, lowerCI, upperCI) %>%
          mutate(dataset = "Random"))
...
Let's do some log rank tests for these curves
```{r}
## Asx
homol_random_Asx_logrank <- lapply(1:1000, function(x){
  broom::glance(survdiff(Surv(Days,EventCensored)~dataset,
          data=perm_data[[x]] %>% 
...
homol_random_Asx_logrank_t <- homol_random_Asx_logrank %>%
  transpose()

homol_random_Asx_logrank_stats <- tibble(chisq = unlist(homol_random_Asx_logrank_t$statistic), p.value = unlist(homol_random_Asx_logrank_t$p.value))


homol_random_Asx_chisq_fig <- homol_random_Asx_logrank_stats %>%
  ggplot() +
  geom_histogram(aes(x = chisq), bins = 100) +
  labs(y = "Count", x = "Chi Squared Test Statistic", title = "Asx") +
  theme_bw()


homol_random_Asx_chisq_fig

saveRDS(homol_random_Asx_chisq_fig, paste0("figures/robj/", CurrentEpitopeName, "_homol_random_Asx_chisq_fig.RDS"))

homol_random_Asx_p_fig <- homol_random_Asx_logrank_stats %>%
  ggplot() +
  geom_histogram(aes(x = p.value), bins = 100) +
  scale_x_continuous(trans = "log10") +
  labs(y = "Count", x = "P value") +
...
homol_random_Asx_p_fig

saveRDS(homol_random_Asx_p_fig, paste0("figures/robj/", CurrentEpitopeName, "_homol_random_Asx_p_fig.RDS"))

median_Asx_logrank <- homol_random_Asx_logrank_stats %>%
  summarise(med_chisq = median(chisq), med_p = median(p.value))
...
## Sx
homol_random_Sx_logrank <- lapply(1:1000, function(x){
  broom::glance(survdiff(Surv(Days,EventCensored)~dataset,
          data=perm_data[[x]] %>% 
...
homol_random_Sx_logrank_t <- homol_random_Sx_logrank %>%
  transpose()

homol_random_Sx_logrank_stats <- tibble(chisq = unlist(homol_random_Sx_logrank_t$statistic), p.value = unlist(homol_random_Sx_logrank_t$p.value))


homol_random_Sx_chisq_fig <- homol_random_Sx_logrank_stats %>%
  ggplot() +
  geom_histogram(aes(x = chisq), bins = 100) +
  labs(y = "Count", x = "Chi Squared Test Statistic", title = "Sx") +
  theme_bw()


homol_random_Sx_chisq_fig

saveRDS(homol_random_Sx_chisq_fig, paste0("figures/robj/", CurrentEpitopeName, "_homol_random_Sx_chisq_fig.RDS"))

homol_random_Sx_p_fig <- homol_random_Sx_logrank_stats %>%
  ggplot() +
  geom_histogram(aes(x = p.value), bins = 100) +
  scale_x_continuous(trans = "log10") +
  labs(y = "Count", x = "P value") +
...
homol_random_Sx_p_fig

saveRDS(homol_random_Sx_p_fig, paste0("figures/robj/", CurrentEpitopeName, "_homol_random_Sx_p_fig.RDS"))

median_Sx_logrank <- homol_random_Sx_logrank_stats %>%
  summarise(med_chisq = median(chisq), med_p = median(p.value))
...
Survival curves with logrank test results
```{r}

homol_random_Asx_fig <- surv_Asx_data_forfig %>%
  ggplot(aes(x = time, y = med_surv, color = dataset)) +
  geom_line(aes(x = time, y = med_surv))+
  geom_line(data = tibble(time = c(0,61,0,61), med_surv = c(1,1,1,1), dataset = c("Homologous","Homologous","Random","Random")))+
  geom_ribbon(aes(x = time, y = med_surv, ymin = lowerCI, ymax = upperCI, fill = dataset), alpha = 0.3, color = NA)+
  ...
  scale_fill_grey(start = 0.6, end = 0.3) +
  labs(color = "", fill = "", y = "Median probability of survival without reinfection", x = "Time in days") +
  annotate("rect", xmin = 235, xmax = 365, ymin = 0.01, ymax = 0.18, color = "black", fill = "white") +
  annotate("text", x = 300, y = 0.13, label = paste0("Median \u03a7\u00b2 = ", round(median_Asx_logrank$med_chisq,3)))+ 
  annotate("text", x = 300, y = 0.06, label = paste0("Median p = ", round(median_Asx_logrank$med_p ,4)))+
  scale_y_continuous(limits = c(0,1))+
  theme_bw()

homol_random_Asx_fig


saveRDS(homol_random_Asx_fig, paste0("figures/robj/", CurrentEpitopeName,"_homolvrandom_survival_Asx.RDS"))




##Sx
homol_random_Sx_fig <- surv_Sx_data_forfig %>%
  ggplot(aes(x = time, y = med_surv, color = dataset)) +
  geom_line(aes(x = time, y = med_surv))+
  geom_line(data = tibble(time = c(0,61,0,61), med_surv = c(1,1,1,1), dataset = c("Homologous","Homologous","Random","Random")))+
  geom_ribbon(aes(x = time, y = med_surv, ymin = lowerCI, ymax = upperCI, fill = dataset), alpha = 0.3, color = NA)+
  ...
  labs(color = "", fill = "", y = "Median probability of survival without reinfection", x = "Time in days") +
  annotate("rect", xmin = 235, xmax = 365, ymin = 0.01, ymax = 0.18, color = "black", fill = "white") +
  annotate("text", x = 300, y = 0.13, label = paste0("Median \u03a7\u00b2 = ", round(median_Sx_logrank$med_chisq,3)))+ 
  annotate("text", x = 300, y = 0.06, label = paste0("Median p = ", round(median_Sx_logrank$med_p ,4))) +
  scale_y_continuous(limits = c(0,1))+
  theme_bw()

homol_random_Sx_fig

saveRDS(homol_random_Sx_fig, paste0("figures/robj/", CurrentEpitopeName,"_homolvrandom_survival_Sx.RDS"))
