...
#knitr::opts_knit$set(root.dir = "~/UNC OneDrive/OneDrive - University of North Carolina at Chapel Hill/UNC Dissertation/Duke Malaria Work/Mozzie Survival")
...
HumanData<-read.csv("epitopes/MozzieAminoAcidAnalysis/Human Demographic Data/spat21_human_final_censored_data_for_dissertation_with_exposure_outcome_1MAR2020.csv") %>%
  mutate(week_year = paste0(week(sample_id_date), "_", year(sample_id_date)))

season <- readRDS("epitopes/MozzieAminoAcidAnalysis/Mosquito demographic data/spat21_mosquito_anopheles_merged_data_18JAN2019.RDS") %>%
...
# CHOOSE EPITOPE
Define current epitope CSP/AMA and AA positions of interest (as determined from the RF models):
```{r,echo=T}
# #CSP CD4 T-cell
...
seqs <- Biostrings::readDNAStringSet("epitopes/MozzieAminoAcidAnalysis/spat21_CSP_uniqueSeqs_final_censored.fasta")


haplotypes<-read.csv("epitopes/MozzieAminoAcidAnalysis/Haplotype Data for Haplotype Prevalence/Summarized data/spat21_csp_summarized_haplotype_list_31DEC2019.csv")
...
seqs <- Biostrings::readDNAStringSet("epitopes/MozzieAminoAcidAnalysis/spat21_AMA_uniqueSeqs_final_censored.fasta")

haplotypes<-read.csv("epitopes/MozzieAminoAcidAnalysis/Haplotype Data for Haplotype Prevalence/Summarized data/spat21_ama_summarized_haplotype_list_31DEC2019.csv")
...
saveRDS(homol_fig, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_homol_nocutoff.RDS"))
...
saveRDS(hazard_plot, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_hazardplot.RDS"))
...
saveRDS(homol_fig_2, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_homol_fig_cutoff60.RDS"))
...
summary(model)
#HR: 0.64 (0.4525028 0.8915091) ***cfm sees .63 (0.4508199 0.887919) This is stochastic because of the random effects added. Will set seed at the top and get final numbers later.
...
coxmetable1
coxplot1=ggplot(coxmetable1,aes(y=label,x=exp(coeff1),xmin=`2.5 %`, xmax=`97.5 %`))+
...
saveRDS(coxplot1, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_coxme_fig.RDS"))
...
saveRDS(freq_compare, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_freq_compare_cutoff60.RDS"))


epitope_props %>%
  arrange(AAs) %>%
  ungroup() %>%
  summarise(wilcoxp = wilcox.test(prop_AAs[cutoff_60 == 1], prop_AAs[cutoff_60 == 0], paired = TRUE)$p.value) #p = 0.556, no sig difference
...
saveRDS(indiv_times_fig, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_individual_strat_homol_time.RDS"))
...
saveRDS(perm_plot, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_perm_data_time_dist.RDS"))
...
saveRDS(homol_random_fig, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_homolvrandom_survival.RDS"))
...
saveRDS(random_logrank_figure, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_random_logrank_chisq_fig.RDS"))
...
saveRDS(random_strata_fig, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_random_strat_survival.RDS"))
...
saveRDS(random_aHR_fig, paste0("epitopes/MozzieAminoAcidAnalysis/figures/", CurrentEpitopeName,"_random_coxme_fig.RDS"))
